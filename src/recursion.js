...
var range = function(x, y) {
    ...
    if (y > x) {
        array.push(x+1);
        var result = array.concat(range(x+1,y));
        return result;
    } else if (x > y) {
        array.push(x-1);
        var result = array.concat(range(x-1, y));
...
var palindrome = function(string) {
    ...
    if(newString[0] === newString[strLength - 1]) {
        return palindrome(newString.slice(1, strLength-1));
    }
    return false;
};
...
var compareStr = function(str1, str2) {
    ...
    if ((str1Length === 1 && str2Length === 1) || (str1Length === 0 && str2Length === 0)) {
        if (str1 === str2) {
            return true;
        } else {
            return false;
...
var reverseArr = function(array) {
    ...
    newArr.push(array[arrLeng - 1]);

    return newArr.concat(reverseArr(array.slice(0, arrLeng-1)));
};
...
var rMap = function(array, callback) {
    ...
    return resultArr.concat(rMap(copyArr.slice(1), callback));
};
...
var augmentElements = function(array, aug) {
    ...
    newArray = newArray.concat(augmentElements(array.slice(1), aug));
    return newArray;
};
...
var numToText = function(str) {
    ...
    var numberDigits = ["zero", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine"];

    if (isNaN(Number(strtoArr[0]))) {
        resultString = resultString + strtoArr[0];
    } else {
        var toNumber = Number(strtoArr[0]);
        strtoArr[0] = numberDigits[toNumber];
        resultString = resultString + strtoArr[0];
    }
    var result = numToText(strtoArr.slice(1).join(' '));
